/*
  # Create celestial bodies table with enhanced content types
  ...
  1. New Tables
    - `celestial_bodies`
      ...
      - `title` (text)
      - `content_type` (enum: movie, book, album, game, other, studying, work)
      - `genre` (text, optional)
      - `creator_id` (text, optional)
      - `series_id` (text, optional)
      - `external_id` (text, optional)
      - `api_data` (jsonb, optional)
      - `position_x`, `position_y`, `position_z` (double precision)
...
DO $$ BEGIN
    CREATE TYPE content_type_enum AS ENUM ('movie', 'book', 'album', 'game', 'other', 'studying', 'work');
EXCEPTION
